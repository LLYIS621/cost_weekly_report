...
def resolve_excel_path():
    ...
    parser.add_argument(
        '--excel',
        help='运营团队BI报表.xlsx 的路径；不传时优先读环境变量 YYZX_EXCEL，再自动查找常见位置。',
    )
    ...
    env_excel = os.environ.get('YYZX_EXCEL')
    if env_excel:
        candidates.append(Path(env_excel))
    candidates.extend([
        BASE_DIR / '运营团队BI报表.xlsx',
        BASE_DIR.parent / '运营团队BI报表.xlsx',
    ])
    ...
    raise FileNotFoundError(
        '未找到运营团队BI报表.xlsx。请把文件放到项目目录/上级目录，'
        '或使用 --excel 指定路径。\n已检查：\n' + checked
    )
...
def read_required_sheet(excel_file, sheet_name):
    if sheet_name not in excel_file.sheet_names:
        ...
        raise ValueError(f'数据源缺少必要 sheet：{sheet_name}。当前可用 sheet：{available}')
    df_sheet = pd.read_excel(excel_file, sheet_name=sheet_name)
    missing = sorted(REQUIRED_SHEET_COLUMNS[sheet_name] - set(df_sheet.columns))
    if missing:
...
df = read_required_sheet(excel_file, PROJECT_SHEET)
df2 = read_required_sheet(excel_file, TEAM_SHEET)
...
dept_keys = ['K2', 'K3', 'K4', 'K4-品商', 'K7', '品策-小红书']
team_data = {}
total_days = len(raw_trend_list)
for dept in dept_keys:
    tdf = df2[(df2['所属中心'] == '运营中心') & (df2['日期粒度'] == '月') & (df2['运营部门'] == dept) & (df2['团队粒度'] == '部门')].sort_values('月')
    if len(tdf) == 0:
        continue
    total = tdf['消耗'].sum()
...
df_roi = read_required_sheet(excel_file, ROI_SHEET)
...
for _, r in mt_cost.iterrows():
    cost_map[r['运营部门']] = {'cost': round(r['消耗'], 2), 'staff': round(r['人效人数'], 2)}
...
month_target_list = []
for dept in dept_keys + ['K部门']:
    cm = cost_map.get(dept, {'cost': 0, 'staff': 0})
...
# 17. CENTER_YEAR_MEDIA: 运营中心年度各媒体消耗占比
center_year_media = daily.groupby('媒体')['消耗'].sum().reset_index().sort_values('消耗', ascending=False)
cym_total = center_year_media['消耗'].sum()
center_year_media_list = [{'media': r['媒体'], 'cost': round(r['消耗'], 2), 'pct': round(r['消耗'] / cym_total * 100, 1)} for _, r in center_year_media.iterrows()]
...
month_center = {'cost': mc_center.get('消耗', 0), 'staff': mc_center.get('人效人数', 0)}
